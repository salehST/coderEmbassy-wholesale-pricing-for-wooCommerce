...
class Controller {
    ...
    public static function routes(): void {
        ...
        /* Starter: no /groups — dealer groups are Business+ */
    ...
    public static function handle_prices(WP_REST_Request $r): WP_REST_Response {
        if ($r->get_method() === 'POST') {
            $d = $r->get_json_params() ?: [];
            if ( Database::is_at_limit() ) {
                return self::json([
                    'error'       => 'limit_reached',
                    'message'     => 'You have reached the ' . CEWP_MAX_RULES . '-rule limit on Starter. Upgrade to Business for unlimited rules.',
                    'upgrade_url' => 'https://plugin.coderembassy.com/',
                ], 403);
            }
            ...
            if ( $id === false ) {
                return self::json(['error'=>'limit_reached','message'=>'Rule limit reached. Upgrade to Business.'], 403);
            }
    ...
    public static function handle_price(WP_REST_Request $r): WP_REST_Response {
        ...
        if ($r->get_method() === 'PUT') {
            if ( ! Database::is_rule_manageable( $id ) ) {
                return self::json([
                    'error'       => 'over_limit',
                    'message'     => 'This rule is above the Starter limit. You have ' . Database::get_rule_count() . ' rules; Starter allows ' . CEWP_MAX_RULES . '. Upgrade to Business to manage all rules.',
                    'upgrade_url' => 'https://plugin.coderembassy.com/',
                ], 403);
            }
            $d = $r->get_json_params() ?: [];
        ...
        $row = (array) $row;
        $row['manageable'] = Database::is_rule_manageable( $id );
    ...
    /** Starter: CSV import (prices only, max 10 rows). */
    public static function handle_import_csv( WP_REST_Request $r ): WP_REST_Response {
        ...
        foreach ( $lines as $idx => $line ) {
            if ( $created >= $max_rows ) {
                $errors[] = [ 'line' => $idx + 1, 'message' => 'Starter limit: max ' . $max_rows . ' price rules. Upgrade for more.' ];
                continue;
            }
            $line_no = $idx + 1;
            ...
            if ( $id ) $created++;
            else $errors[] = [ 'line' => $line_no, 'message' => 'Rule limit reached' ];
        }
