 ...
 *
 * FREE:  Per-product price rules (user / group / role scope),
 *        fixed / percent_discount / fixed_discount types,
 *        wholesale sale price, min-qty price tiers, date ranges.
 *
 * PRO (delegated to Pro\GlobalDiscount, Pro\CategoryDiscount):
 *        Global % discount, category % discount, qty-tiered discounts.
 *
 * Resolution order (highest priority first):
 *   1. Exact user rule
 *   2. Dealer group rule
 *   3. WordPress role rule  ← product-level
 *   4. [PRO] Category % discount
 *   5. [PRO] Global % discount
 *   6. WooCommerce default
 */
class Pricing {
    ...
    public static function resolve( float $base, int $product_id, int $variation_id = 0, int $qty = 1 ): ?float {
        ...
        // 4-5: [PRO] category & global discounts handled by Pro\GlobalDiscount filter
        // Those filters run at priority 25 (after this at 20) so they fall through naturally.
