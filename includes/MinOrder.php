...
/**
 * MinOrder – enforces minimum order rules for wholesale customers.
 *
 * PRO feature: min subtotal, min total qty, block checkout until met.
 * FREE: Just shows a notice; doesn't block checkout.
 */
class MinOrder {
    ...
    public static function check_cart(): void {
        ...
        if ( $rule->min_subtotal > 0 && $subtotal < $rule->min_subtotal ) {
            ...
            if ( cewp_is_pro() && $rule->require_before_whl ) {
